...
def index():
    if request.method =="GET":
        ...
        #get soup based on supplied search query
        ...
        # for each item, check how much the name matches the original search
        for item in items:
        # get the match ratio
            ration = difflib.SequenceMatcher(None, name.lower(), item['name'].lower()).ratio() 
            
            # if the ration is above half
            if(ration > 0.5):
                #  if the ratio is above half simply assign the item as the current match
                if(current_match is None): 
                    current_match = item
                #otherwise, compare the price of the current 
